...
class Config:
    ...
    # ------------------------------------------------------------
    # Quality evaluation / state options
    # ------------------------------------------------------------
    # If True: compute quality by Exact Match (EM) between the model response
    # and the dataset ground-truth output (requires prompts to carry ground_truth).
    # In this mode we do NOT inject per-model quality into the state.
    USE_EM_EXACT_MATCH = False

    # EM comparison options (kept lightweight; you can tighten/relax as needed)
    EM_NORMALIZE = True          # lower + strip punctuation/articles + normalize whitespace

    # Whether to include quality scores as part of the state vector.
    # For your requested behavior: keep this False when USE_EM_EXACT_MATCH=True.
    INCLUDE_QUALITY_IN_STATE = not USE_EM_EXACT_MATCH
